...
class AccountCsvRepositoryTest {
    ...
    void upsertAllCreatesCsvAndReplacesExistingAccountsByAccountId() {
        AccountCsvRepository repository = new AccountCsvRepository(TestFixtures.properties(dataDirectory));

        repository.upsertAll(List.of(
                new AccountCsvRecord("acc-1", "item-1", "Checking", "1111", "depository", "checking", "true")));
        repository.upsertAll(List.of(
                new AccountCsvRecord("acc-1", "item-1", "Renamed Checking", "1111", "depository", "checking", "true"),
                new AccountCsvRecord("acc-2", "item-1", "Savings", "2222", "depository", "savings", "false")));
