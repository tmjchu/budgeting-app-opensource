...
class PlaidConnectionServiceTest {
    ...
    @Mock
    private PlaidItemCsvRepository plaidItemRepository;
    @Mock
    ...
    void exchangePublicTokenStoresPlaidItemAndSelectedAccounts() {
        ...
        PlaidConnectionService service = new PlaidConnectionService(
                plaidGateway,
                plaidItemRepository,
                accountService,
        ...
        ExchangePlaidPublicTokenResult result = service.exchangePublicToken(new ExchangePlaidPublicTokenCommand(
                "public-token",
                "Test Bank",
                List.of(new SelectedAccountCommand("acc-1", "Checking", "1234", "depository", "checking"))));

        assertThat(result.plaidItem().accessToken()).isEqualTo("access-token");
        assertThat(result.trackedAccounts()).hasSize(1);
        ArgumentCaptor<PlaidItemCsvRecord> itemCaptor = ArgumentCaptor.forClass(PlaidItemCsvRecord.class);
